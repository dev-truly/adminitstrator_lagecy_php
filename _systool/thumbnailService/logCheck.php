...
<html>
	...
	<body>
		<h1 style="text-align:center;">썸네일 작업 사이트 로그 확인</h1>
		<form name="searchArea" method="post" action="<?=$_SERVER['PHP_SELF']?>">
			<div style="text-align:center;">
				<select name="site">
					<option value="">작업사이트</option>
					<?php
				...
				<input type="button" name="processPage" onclick="location.href='./'" value="작업페이지" />
				<?php
						if ($site) {
				?>
							<input type="button" name="processPage" onclick="location.href='./logFileDown.php?site=<?=$site?>'" value="로그 다운" />
				<?php
							}
				?>
			</div>
				...
				<tbody>
					<?php
						foreach ($arraySiteLogFile as $logFileName) {
							...
							$historyName		= floor($arrayHistoryName[1]) . ' 번째 상품 부터 300개 상품의 ';  
