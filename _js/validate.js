...
/*****************NULL체크 및 정규식 검사************************/	
	for(i = 0; i < validate.length; i++){
		...
		if(require=="on"){
			if($jq.trim(validBox.val()).length == 0){
					alert(msg + '을(를) 입력하세요',validBox);
					$jq(validBox).css('background-color',bgcolor);
					resetFocus(validBox) 
						//$jq(validBox).css('background-image','url("'+bgimage1+'")');
					return false;
				}else{
				$jq(validBox).css('background-color','#ffffff');
				//$jq(validBox).css('background-image','none');
				}
				for(k=0;k<protoType.length;k++){
					if(validOption[0][j] == protoType[k][0]){
						var regExp=protoType[k][1];
						if( !regExp.test($jq.trim(validBox.val()))  ){
							alert("올바른 " + msg+"(을)를 입력하세요",validBox);
							$jq(validBox).css('background-color',bgcolor);
							//$jq(validBox).css('background-image','url("'+bgimage2+'.jpg")');
							resetFocus(validBox) 
							return false;
						}else{
						$jq(validBox).css('background-color','#ffffff');
				...
				for(k=0;k<protoType.length;k++){
					if(validOption[0][j] == protoType[k][0]){
						...
						if( !regExp.test($jq.trim(validBox.val()))  ){
							alert("올바른 " + msg +"(을)를 입력하세요",validBox);
							$jq(validBox).css('background-color',bgcolor);
							//$jq(validBox).css('background-image','url("'+bgimage2+'")');
							resetFocus(validBox) 
							return false;
						}else{
						$jq(validBox).css('background-color','#ffffff');
...
var mailList = new Array(
	...
	for(i = 0; i < changeMail.length; i++){
		...
		$jq('<option value="">직접입력</option>').appendTo(validBox);
		for(k=0;k<mailList.length;k++){
			$jq('<option value='+mailList[k]+'>'+mailList[k]+'</option>').appendTo(validBox);
		}
	}
...
function func_cell(hpSel){
	...
	'019'	
		...
		for(k=0;k<hpList.length;k++){
			$jq('<option value='+hpList[k]+'>'+hpList[k]+'</option>').appendTo(validBox);
		}
...
function func_tel(telSel){
	...
	'064'
		...
		for(k=0;k<telList.length;k++){
			$jq('<option value='+telList[k]+'>'+telList[k]+'</option>').appendTo(validBox);
		}
		//validBox.css('width','60px');
	}
}//fe
...
var routeList = new Array(
	...
	for(i = 0; i < routeSel.length; i++){
		...
		for(k=0;k<routeList.length;k++){
			$jq('<option value='+routeList[k][1]+'>'+routeList[k][0]+'</option>').appendTo(validBox);
		}
	}
...
for(i = 0; i < areaSel.length; i++){
		...
		for(k=0;k<areaList.length;k++){
			$jq('<option value='+areaList[k][1]+'>'+areaList[k][0]+'</option>').appendTo(validBox);
		}
	}
	//validBox.css('width','60px');
...
var QuestionList = new Array(
	...
	for(i = 0; i < changeQuestion.length; i++){
		...
		for(k=0;k<QuestionList.length;k++){
			$jq('<option value='+QuestionList[k]+'>'+QuestionList[k]+'</option>').appendTo(validBox);
		}
	}
		...
		valiname = numsetting[i][0];
		 ...
		 for (j=0;j<validOption.length;j++ ){
				 ...
				 for(k=0;k<option.length;k=k+2){
					  ...
					  option[k+1]=option[k+1].replace("'","");
					 $jq('#'+valiname).attr(option[k],option[k+1]);
				 }
		 }
...
function checkCount(chName,min,max,msg){
	var check = $jq('input[name='+chName+']');
	var count = 0;
	check.each(function(){
...
// 아래 css 는 date picker 의 화면을 맞추는 코드입니다.
	...
	var pasteEventName = ($jq.browser.msie ? 'paste' : 'input') + ".mask";
	...
	$jq.fn.extend({
		...
		mask: function(mask, settings) {
			if (!mask && this.length > 0) {
				...
				return $jq.map(input.data("buffer"), function(c, i) {
			...
			var defs = $jq.mask.definitions;
			var tests = [];
			var partialPosition = mask.length;
			var firstNonMaskPos = null;
			var len = mask.length;

			$jq.each(mask.split(""), function(i, c) {
				if (c == '?') {
					len--;
					partialPosition = i;
				} else if (defs[c]) {
			...
			return this.each(function() {
				var input = $jq(this);
				var buffer = $jq.map(mask.split(""), function(c, i) { if (c != '?') return defs[c] ? settings.placeholder : c });
...
//메일선택 변경시 input 변경
$jq('#changeMail').change(function(){
var target = $jq(this).attr('changeMail');
$jq('#'+target).val( $jq(this).val()  );
});
//질문 select 변경시 질문 input 변경
$jq('#changeQuestion').change(function(){
var target = $jq(this).attr('changeQuestion');
$jq('#'+target).val( $jq(this).val()  );
});
...
var changeMail = $jq('#changeMail');
func_changeMail(changeMail); 
//메일선택적용

var hpSel = $jq('*[hpSel=on]');
func_cell(hpSel);
//핸드폰 번호

var telSel = $jq('*[telSel=on]');
func_tel(telSel);
//전화번호 국번

var routeSel = $jq('*[routeSel=on]');
func_routeSel(routeSel);
//가입경로


var areaSel = $jq('*[areaSel=on]');
func_areaSel(areaSel);
//지역[국내]


var requireCheck=$jq('*[require]');
func_require(requireCheck);
//필수 입력 


var changeQuestion=$jq('#changeQuestion');
